...
async function addProductToCart(req, res) {
    ...
    try {
        await cart_service_1.default.postProductToCart(id, name, price, image, amount, userId);
        res.sendStatus(201);
    }
    catch (err) {
...
async function getProductsToCart(req, res) {
    ...
    try {
        const products = await cart_service_1.default.getProductsToCart(userId.toString());
